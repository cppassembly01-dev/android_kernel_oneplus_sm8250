...
static int a6xx_gmu_poll_with_retry(struct kgsl_device *device, u32 offset,
		u32 expected, unsigned int timeout, u32 mask,
		const char *op_name)
{
	...
	/*
	 * A650-family bringup can occasionally miss the first poll window due
	 * to interconnect and posted-write latency around reset deassertion.
...
static int a6xx_gmu_start(struct kgsl_device *device)
{
	...
	if (a6xx_gmu_poll_with_retry(device,
