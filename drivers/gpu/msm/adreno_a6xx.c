...
static unsigned int a6xx_validate_min_access_length(struct kgsl_device *device,
		unsigned int mal)
{
	if (mal == 32 || mal == 64)
		return mal;
...
static unsigned int a6xx_validate_ubwc_mode(struct kgsl_device *device,
		unsigned int mode)
{
	switch (mode) {
	case KGSL_UBWC_1_0:
...
static void a6xx_start(struct adreno_device *adreno_dev)
{
	...
	if (of_property_read_u32(device->pdev->dev.of_node,
		...
		mal = 32;
	mal = a6xx_validate_min_access_length(device, mal);

	if (of_property_read_u32(device->pdev->dev.of_node,
		"qcom,ubwc-mode", &mode))
		mode = KGSL_UBWC_2_0;
	mode = a6xx_validate_ubwc_mode(device, mode);
